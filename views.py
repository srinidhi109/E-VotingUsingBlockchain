...
def getOutput(status):
    ...
    con = pymysql.connect(host='127.0.0.1',port = 3308,user = 'root', password = 'root', database = 'evoting',charset='utf8')
    with con:
        cur = con.cursor()
...
def ValidateUser(request):
    if request.method == 'POST':
        ...
        img = cv2.imread('C:/Python/EVoting/EVotingApp/static/photo/test.png')
...
def AddPartyAction(request):
    if request.method == 'POST':
      ...
      db_connection = pymysql.connect(host='127.0.0.1',port = 3308,user = 'root', password = 'root', database = 'evoting',charset='utf8')
...
def Signup(request):
    if request.method == 'POST':
      ...
      fs = FileSystemStorage()
      filename = fs.save('C:/Python/EVoting/EVotingApp/static/profiles/'+username+'.png', myfile)
